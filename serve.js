...
const dbPath = path.join(__dirname, 'voxlibrary.db');
let db;
try {
...
const server = http.createServer((incomingReq, serverRes) => {
  ...
  // API: Get community voices from local database
  if (incomingReq.method === 'GET' && incomingReq.url === '/api/community-voices') {
    try {
      const voices = db.prepare('SELECT * FROM voice_samples WHERE is_public = 1 ORDER BY created_at DESC').all();
      serverRes.writeHead(200, { 'Content-Type': 'application/json' });
      serverRes.end(JSON.stringify(voices));
    } catch (err) {
      console.error('Database error:', err);
      serverRes.writeHead(500, { 'Content-Type': 'application/json' });
      serverRes.end(JSON.stringify({ error: 'Failed to fetch voices' }));
    }
    return;
  }
  ...
  // API: Submit voice sample
  if (incomingReq.method === 'POST' && incomingReq.url === '/api/submit-voice') {
    let body = '';
    incomingReq.on('data', chunk => body += chunk);
    incomingReq.on('end', () => {
      try {
        const data = JSON.parse(body);
        const { name, accent, pitch, describe1, describe2, contributor_id } = data;
        
        const voice_id = uuidv4();
        const stmt = db.prepare(`
          INSERT INTO voice_samples (voice_id, name, accent, pitch, describe1, describe2, contributor_id, is_public)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        `);
        stmt.run(voice_id, name, accent, pitch, describe1, describe2, contributor_id || 'anonymous', 1);
        
        console.log('Voice submitted:', name, 'ID:', voice_id);
        serverRes.writeHead(200, { 'Content-Type': 'application/json' });
        serverRes.end(JSON.stringify({ success: true, voice_id }));
      } catch (err) {
